...
const withPropsLogger = <
  T extends Component,
  OriginalProps extends object = GetComponentProps<T>
>(
  WrappedComponent: Constructor<T>
) => {
  // type OriginalProps = GetComponentProps<T>
  // (2)
  ...
  class WithPropsLogger extends Component<Props> {
    ...
    render() {
      // we use pickWithRest 👉 all of this is unfortunately needed because TS issue with spreading Generics https://github.com/Microsoft/TypeScript/issues/15792
      const { forwardedRef, rest } = pickWithRest<Props, PrivateProps>(
        this.props,
        ['forwardedRef']
      )
